...
addpath('/users/staff/jppaquin/NEMO_PREPARATION/0_common/m_map')
...
% - DESTINATION grid files required
dirbathy='/users/staff/jppaquin/scratch/CONFIGURATION_SCOTIAN_JP';
filebathy='bathy_meter_Scotian.nc';
dircoord='/users/staff/jppaquin/scratch/CONFIGURATION_SCOTIAN_JP';
filecoord='coordinates_SCOTIAN.nc';
dirmesh ='/users/staff/jppaquin/scratch/CONFIGURATION_SCOTIAN_JP';
...
globU    =f_readnetcdf(dir,Ufile,'vozocrtx',cdfversion); globU=squeeze(globU);
...
globSSH=f_readnetcdf(dir,SSHfile,'sossheig',cdfversion);
globSSH=squeeze(globSSH);
reg_glob_dataSSH(1,:,:,1)=squeeze(globSSH(index_minjj:index_maxjj,index_minii:index_maxii));
clear globSSH

% -- EXTRACT T
display([ '      LOAD   T data for Y=' num2str(year) ' M=' month ])
Tfile=([ inputdata '_ORCA025_' num2str(year) month '_gridT.nc']);
globT=f_readnetcdf(dir,Tfile,'votemper',cdfversion);
globT=squeeze(globT);
reg_glob_dataT(:,:,:,1)=squeeze(globT(:,index_minjj:index_maxjj,index_minii:index_maxii));
clear globT

% -- EXTRACT S
display([ '      LOAD   S data for Y=' num2str(year) ' M=' month ])
Sfile=([ inputdata '_ORCA025_' num2str(year) month '_gridS.nc']);
globS=f_readnetcdf(dir,Sfile,'vosaline',cdfversion);
globS=squeeze(globS);
reg_glob_dataS(:,:,:,1)=squeeze(globS(:,index_minjj:index_maxjj,index_minii:index_maxii));
clear globS

% -- EXTRACT U
display([ '      LOAD   U data for Y=' num2str(year) ' M=' month ])
Ufile=([ inputdata '_ORCA025_' num2str(year) month '_gridU.nc']);
globU=f_readnetcdf(dir,Ufile,'vozocrtx',cdfversion);
globU=squeeze(globU);
reg_glob_dataU(:,:,:,1)=squeeze(globU(:,index_minjj:index_maxjj,index_minii:index_maxii));
clear globU

% -- EXTRACT V
display([ '      LOAD   V data for Y=' num2str(year) ' M=' month ])
Vfile=([ inputdata '_ORCA025_' num2str(year) month '_gridV.nc']);
globV=f_readnetcdf(dir,Vfile,'vomecrty',cdfversion);
globV=squeeze(globV);
reg_glob_dataV(:,:,:,1)=squeeze(globV(:,index_minjj:index_maxjj,index_minii:index_maxii));
clear globV
