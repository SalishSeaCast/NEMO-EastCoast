...
% DEFINE DIMENSIONS AND ATTRIBUTES
  ...
  netcdf.putAtt(ncout,globatt,'Description','Monthly OBC FROM GLORYS2V3')
  netcdf.putAtt(ncout,globatt,'Author','JP Paquin')
  ...
  netcdf.putAtt(ncout,globatt,'Title','MONTHLY OBC')
...
% IMPORTANT NOTE :
%           FOR CONCEPTS CODE VERSION 1.1.0: THE OBCs NEMO REQUIRES 
%               THE INDICES TO INCREASE FROM THE EDGE OF THE DOMAIN 
%               INWARD. THEREFORE, INDICES MUST BE INVERTED FOR THE 
%               NORTHERN AND EASTERN BOUNDARIES (only)
%           FOR CONCEPTS VERSION 2.0.0 and ABOVE, NO INVERSION IS REQUIRED
if strcmp(concepts_version,'1.1.0')
   if     strcmp(obc_name,'north')
     display([ 'INVERSION OF Y FOR :' obc_name])
     tmpAlon=Alon(NY:-1:1,1:NX);               Alon=tmpAlon;
     tmpAlat=Alat(NY:-1:1,1:NX);               Alat=tmpAlat; 
     tmpMask=mask(1:NZ,NY:-1:1,1:NX);          mask=tmpMask;
     tmpdatain=datain(1:NZ,NY:-1:1,1:NX,1:NT); datain=tmpdatain;
     clear tmpAlon tmpAlat tmpMask tmpdatain
   elseif strcmp(obc_name,'east')
     display([ 'INVERSION OF X FOR :' obc_name ])  
     tmpAlon=Alon(1:NY,NX:-1:1);               Alon=tmpAlon;
     tmpAlat=Alat(1:NY,NX:-1:1);               Alat=tmpAlat; 
     tmpMask=mask(1:NZ,1:NY,NX:-1:1);          mask=tmpMask;
     tmpdatain=datain(1:NZ,1:NY,NX:-1:1,1:NT); datain=tmpdatain;
     clear tmpAlon tmpAlat tmpMask tmpdatain     
   end
end
...
netcdf.close(ncout)
  ...
  display('***** OLDER VERSIONS NETCDF NOT DONE *****')
  isok=0;
  return
end
