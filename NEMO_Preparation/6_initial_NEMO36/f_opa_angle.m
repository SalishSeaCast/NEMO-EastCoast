...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%   JPPAQUIN : OPTIMIZED CODE FOR ROTATION ...
%for jj=2:jpj-1
%   for ji=2:jpi
%             ! north pole direction & modulous (at t-point)
zlam = glamt(2:jpj-1,2:jpi);
zphi = gphit(2:jpj-1,2:jpi);
zxnpt = - 2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi );
zynpt = - 2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi );
znnpt = zxnpt.*zxnpt + zynpt.*zynpt;
clear zlam zphi            

%             ! north pole direction & modulous (at u-point)
zlam = glamu(2:jpj-1,2:jpi);
zphi = gphiu(2:jpj-1,2:jpi);
zxnpu = 0. - 2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi );
zynpu = 0. - 2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi );
znnpu = zxnpu.*zxnpu + zynpu.*zynpu;
clear zlam zphi

%             ! north pole direction & modulous (at v-point)
zlam = glamv(2:jpj-1,2:jpi);
zphi = gphiv(2:jpj-1,2:jpi);
zxnpv = - 2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi );
zynpv = - 2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi );
znnpv = zxnpv.*zxnpv + zynpv.*zynpv;
clear zlam zphi

%             ! north pole direction & modulous (at f-point)
zlam = glamf(2:jpj-1,2:jpi);
zphi = gphif(2:jpj-1,2:jpi);
zxnpf = - 2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi );
zynpf = - 2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi );
znnpf = zxnpf.*zxnpf + zynpf.*zynpf;

%             ! j-direction: v-point segment direction (around t-point)
zlam = glamv(2:jpj-1,2:jpi);
zphi = gphiv(2:jpj-1,2:jpi);
zlan = glamv(1:jpj-2,2:jpi);
zphh = gphiv(1:jpj-2,2:jpi);
zxvvt =  2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi )   ...
      -  2. .* cos( rad .* zlan ) .* tan( rpi/4. - (rad/2.) .* zphh );
zyvvt =  2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .* zphi )   ...
      -  2. .* sin( rad .* zlan ) .* tan( rpi/4. - (rad/2.) .* zphh );
...
zlam = glamf(2:jpj-1,2:jpi);
zphi = gphif(2:jpj-1,2:jpi);
zlan = glamf(1:jpj-2,2:jpi);
zphh = gphif(1:jpj-2,2:jpi);
zxffu =  2. .* cos( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* cos( rad .* zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
zyffu =  2. .* sin( rad .* zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* sin( rad .* zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
...
zlam = glamf(2:jpj-1,2:jpi);
zphi = gphif(2:jpj-1,2:jpi);
zlan = glamf(2:jpj-1,1:jpi-1);
zphh = gphif(2:jpj-1,1:jpi-1);
zxffv =  2. .* cos( rad.*zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* cos( rad.*zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
zyffv =  2. .* sin( rad.*zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* sin( rad.*zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
...
zlam = glamu(3:jpj  ,2:jpi);
zphi = gphiu(3:jpj  ,2:jpi);
zlan = glamu(2:jpj-1,2:jpi);
zphh = gphiu(2:jpj-1,2:jpi);
zxuuf =  2. .* cos( rad.*zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* cos( rad.*zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
zyuuf =  2. .* sin( rad.*zlam ) .* tan( rpi/4. - (rad/2.) .*zphi )   ...
      -  2. .* sin( rad.*zlan ) .* tan( rpi/4. - (rad/2.) .*zphh );
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%   JPPAQUIN : NOT OPTIMIZED CODE FOR ROTATION ...
% for jj=2:jpj-1
%    for ji=2:jpi
% %             ! north pole direction & modulous (at t-point)
%             zlam = glamt(jj,ji);
